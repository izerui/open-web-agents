...
一个基于 **claude-agent-sdk** 的智能体平台:可视化地定义**特定场景的专用智能体**(如"视频生成智能体""课程设计智能体");每个智能体都对外提供**一套统一接口**,调用方能拿到 **① 运行过程监控 + ② 结构化输出结果**,从而对接企业应用——既支持后台接口(Java/Go/Python/C)对接,也支持企业前端 chatbot 集成。
...
- **让企业自己"造员工"**:每个智能体 = 一个只干特定某件事的专精员工,有自己的提示词、技能、MCP、工具、模型,以及**可选的结构化输出契约**。
...
|---|---|
| 一家公司 | 平台 |
| 一个员工 / 岗位 | 智能体 Agent(模板) |
| 员工的大脑与通用能力 | claude-agent-sdk 引擎 |
| 岗位职责 + 专业技能 + 工具 | 智能体配置(prompt/skills/mcp/tools) |
...
| **Agent(智能体/模板)** | 可配置的专用 agent 定义,平台核心一等公民 | name/icon/description、systemPrompt、skills、mcpServers、tools、subagents、model、capabilities、**inputSchema(可选)**、**outputSchema(可选)**、verifyRules(可选)、accessControl |
| **Project(项目)** | 组织容器,可带共享上下文/知识 | name、systemPrompt(可选)、knowledge(可选) |
| **Session(会话)** | 用某智能体发起的一次运行实例 | agentId、projectId、workspaceDir、状态、sdkSessionId(resume 用) |
...
| **StructuredResult** | 按智能体 outputSchema 校验后的 JSON | 仅当智能体定义了 outputSchema 时存在 |
| **ApiKey** | 对外调用凭证 | 归属智能体/用户、配额、scope |
...
**不区分"人用"和"系统用",只有一套统一的"智能体运行接口"。**

- 网页对话、可嵌入 widget、第三方后端(Java/Go/Python/C),都是**同一个运行接口的不同客户端**,后端不分两套。
- **人用 vs 系统用的唯一区别 = 智能体是否定义了 `outputSchema`**:
  - **特定智能体**(定义了 outputSchema):运行结束返回**结构化 JSON 结果**(系统可直接消费)+ 全程过程可监控。
  - **通用智能体**(未定义 outputSchema):只能拿到最终完成的**对话文本**,无结构化结果。
- 因此 `outputSchema` 是智能体的一个**可选能力**,在**构建智能体时定义**;它决定了这个智能体能不能被企业系统当"接口"用。

这条原则直接决定 API 设计:**一套 session/run 内核 + 一套统一 API + 智能体级可选 outputSchema**,而不是 chat API 与 invoke API 各一套。
...
- **agents**: id, ownerId, name, icon, description, config(JSON: systemPrompt/skills/mcpServers/tools/subagents/model/capabilities/effort/maxTurns), inputSchema(JSON?), outputSchema(JSON?), verifyRules(JSON?), webhookUrl?, visibility, createdAt, updatedAt
- **sessions**(= 项目 = 工作目录,一对一): id, agentId, ownerId?(人用时), callerApiKeyId?(系统用时), workspaceDir, sdkSessionId?, title, status, baseUrl?/keyEnc?/model?(会话级覆盖), createdAt
...
- **api_keys**: id, agentId?, ownerId, hashedKey, name, quota(JSON), lastUsedAt, createdAt
...
借鉴 question-bank 的 `agent-engine` 分层思路(通用引擎 / 业务解耦)**全新实现**,并从智能体配置动态构建,而非写死。

### 6.1 从智能体配置动态构建 AgentSpec

```
Agent.config ──buildSpec()──▶ AgentSpec {
...
```
POST /api/agents/{agentId}/invoke      (Header: X-Api-Key)
  body: {
...
POST webhook(可选,智能体配置回调地址)        // 完成时推 { taskId, status, structured, artifacts }
```
...
- **鉴权分层**:平台内人类走 NextAuth session;对外调用走 per-智能体/per-调用方 API Key + 配额。
...
- 角色:admin / user。
- **通用 AccessControl**(open-webui 范式):一张 `access_grants` 表 + 一个前端组件,覆盖 agents/projects/sessions;`*` = public。
...
| **open-webui** | 通用 AccessControl、Folder=项目容器、Model 编辑器=智能体构建器、Valves、MCP=tool server transport、命令面板、SSE 事件语义 | 砍重型:pipelines/functions 三型、评测体系、频道/笔记/日历、30 搜索引擎、集中式 config.py |
...
│  │  ├─ agent/             # 智能体配置 → AgentSpec(buildSpec)
...
**MVP(第一期)——把"造智能体 → 跑会话 → 统一接口拿结构化结果"闭环打通:**

- 登录 + 每用户自带 base_url/key(三级覆盖:invoke > 会话 > 用户默认)
- 智能体构建器(prompt/skills/mcp/tools/model + inputSchema/outputSchema,**schema 先写 JSON**)
...
4. **outputSchema 是接口契约的核心**:决定智能体能否被系统消费,构建器里必须让用户能定义并预览。
...
1. **智能体构建器**里 inputSchema/outputSchema 的编辑形态:表单可视化搭 schema,还是直接写 JSON Schema / zod?
2. **通用智能体**是否也允许无 schema 直接对外 invoke(只回文本),还是对外接口强制要求 outputSchema?
3. **项目 Project** 是否 MVP 就要,还是先只有"智能体 + 会话"两层?
