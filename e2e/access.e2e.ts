...
// 域逻辑的单测覆盖了 canInvokeAssistant 的判定表,但覆盖不了"路由到底有没有调它"——
// 而真实的漏洞恰恰是【授权体系存在但运行路径没接上】。只有从 HTTP 打进去才测得出来。
//
// 这一整组曾经全是绿的,而攻击者用一个普通账号就能:
//   跑别人的私有助手拿到 systemPrompt 与知识库正文 / 枚举并吊销全平台的 API Key。
// 它们此前只在我手工跑的 shell 脚本里验证过 —— 也就是说,下次谁改坏了没人会知道。

import { beforeAll, describe, expect, it } from "vitest";
import { Client, newUser, serverUp } from "./client";

type AssistantRes = { assistant?: { id: string }; error?: string };
type ListRes = { assistants?: { id: string; name: string }[] };
...
describe("越权攻击面", () => {
  ...
  beforeAll(async () => {
    ...
    const created = await victim.post<AssistantRes>("/api/assistants", {
      name: `私有助手-${Date.now()}`,
      config: { systemPrompt: "这是绝密提示词", model: "sonnet" },
    });
    expect(created.status).toBeLessThan(300);
    secretId = created.body.assistant?.id ?? "";
    expect(secretId).toBeTruthy();
  });

  it("受害者自己能跑自己的助手(对照组 —— 防止全靠拒绝拿满分)", async () => {
    const r = await victim.post<SessionRes>("/api/sessions", { assistantId: secretId });
    expect(r.status).toBe(201);
  });

  it("攻击者仅凭 id 建不了会话 —— 私有助手不可运行", async () => {
    const r = await attacker.post<SessionRes>("/api/sessions", { assistantId: secretId });
    expect(r.status).toBe(403);
  });

  it("攻击者的助手列表里看不到它", async () => {
    const r = await attacker.get<ListRes>("/api/assistants");
    expect((r.body.assistants ?? []).map((a) => a.id)).not.toContain(secretId);
  });

  it("攻击者不能签一把绑定到受害者助手的 key", async () => {
    const r = await attacker.post<KeyRes>("/api/keys", { name: "steal", assistantId: secretId });
    // 与"助手不存在"同码:这个接口不能变成 id 存在性预言机
    expect(r.status).toBe(404);
  });

  it("攻击者不能改受害者的助手定义", async () => {
    const r = await attacker.post<AssistantRes>("/api/assistants", {
      id: secretId,
      name: "被篡改",
  ...
  it("攻击者读不到受害者助手的知识库", async () => {
    const r = await attacker.get(`/api/assistants/${secretId}/knowledge`);
    expect(r.status).toBeGreaterThanOrEqual(400);
  });

  it("攻击者不能把受害者的助手分享给自己", async () => {
    const r = await attacker.post(`/api/assistants/${secretId}/share`, {
...
describe("分享闭环:授予后可用,撤销后立刻失效", () => {
  ...
  beforeAll(async () => {
    ...
    const created = await owner.post<AssistantRes>("/api/assistants", {
      name: `分享测试-${Date.now()}`,
      config: { systemPrompt: "p", model: "sonnet" },
    });
    assistantId = created.body.assistant?.id ?? "";
  });

  it("撤销之前:分享 → 对方可运行", async () => {
    const email = (await mate.get<MeRes>("/api/auth")).body.user?.email ?? "";
    const g = await owner.post<GrantRes>(`/api/assistants/${assistantId}/share`, {
      target: email,
      permission: "read",
    });
    ...
    const s = await mate.post<SessionRes>("/api/sessions", { assistantId });
    expect(s.status).toBe(201);
  });

  it("撤销之后:立刻不可运行 —— 不能只是从列表里隐藏", async () => {
    const r = await owner.del(`/api/assistants/${assistantId}/share?grantId=${grantId}`);
...
describe("未登录一律拒绝", () => {
  ...
  it("伪造的 API Key 被拒", async () => {
    const r = await anon().get("/api/assistants", { "X-Api-Key": "owa_sk_deadbeefdeadbeef" });
    expect(r.status).toBe(401);
  });
});
