...
[MCP 规范支持 OAuth 2.1](https://modelcontextprotocol.io/specification/2025-03-26/basic/authorization) 用于授权。SDK 不会打开浏览器或运行交互式 OAuth 流程。当配置的服务器返回授权质询且没有可用的存储令牌时，代理运行将继续而不使用该服务器的工具，并且该服务器在 [系统初始化消息](/zh-CN/agent-sdk/typescript#sdksystemmessage) 的 `mcp_servers` 数组中报告状态为 `needs-auth`。如果您的代理依赖于特定服务器的连接，请在启动时检查该数组。
...
MCP 服务器连接默认超时为 30 秒。如果您的服务器需要更长时间才能启动，连接将失败。使用 [`MCP_TIMEOUT`](/zh-CN/env-vars) 环境变量提高限制，单位为毫秒。对于需要更多启动时间的服务器，还应考虑：
...
* **[MCP 输出限制和警告](/zh-CN/mcp#mcp-output-limits-and-warnings)**：SDK 如何处理超过 `MAX_MCP_OUTPUT_TOKENS` 的工具结果，包括持久化到磁盘的回退和 `anthropic/maxResultSizeChars` 按工具注解
